...
class HockeyScene(Scene):
    """Mode Hockey 1v1 contre l'IA (module isolé)"""
    ...
    def __init__(self, game):
        ...
        self.rink_rect = pygame.Rect(
            ...
            settings.HEIGHT - top_margin - bottom_margin,
        )
        ...
        self.ai_speed = 380
        self.ai_shoot_power = 820
        self.ai_shoot_cooldown_ms = 800
        self.serve_timer = 0
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_m:
                self.sfx.stop()  # Arrêter les sons du hockey
    ...
    def update(self, dt):
        ...
        if self.serve_timer > 0:
            ...
            if self.serve_timer <= 0:
                self.serve_timer = 0
                # Lancer la rondelle après le délai
    ...
    def _update_ai(self, dt_sec):
        ...
        # Si coincé dans un angle, sortir vers le centre
        in_corner = False
        ...
        # Si le palet est coincé dans un coin gauche, ne pas foncer dedans
        puck_in_left_corner = (
        ...
        if in_corner or puck_in_left_corner:
            ...
            # Sortie sécurisée vers le centre
            ...
            # Si le palet est trop proche du but IA, défendre
        ...
        if dist > 10:
            dx /= dist
            dy /= dist
            self.ai.x += dx * speed * dt_sec
            self.ai.y += dy * speed * dt_sec
    ...
    def _handle_collision(self, player, can_shoot=False, is_ai=False):
        ...
        if dist < collision_dist:
            ...
            if can_shoot:
                if is_ai and self.ai_shoot_cooldown <= 0:
                    # IA tire vers le but gauche (du joueur)
                    target_x = self.rink_rect.left + 10
                    target_y = self.rink_rect.centery + random.uniform(-60, 60)
                    shoot_power = self.ai_shoot_power
                    self.ai_shoot_cooldown = self.ai_shoot_cooldown_ms
                    self.ai_react_timer = 150
                elif not is_ai and self.shoot_cooldown <= 0:
                    # Joueur tire vers le but droit (de l'IA)
            ...
            # Frappe normale
            hit_power = 900
            # Empêcher l'IA d'envoyer vers son propre but
            if is_ai and nx > 0:
                nx = -abs(nx)
    ...
    def _check_goals(self):
        ...
        if self.puck.x - r <= left_line and self.puck.vx < -min_goal_speed:
            if abs(self.puck.y - goal_y) < (goal_h - goal_margin):
                self.ai_score += 1
        ...
        elif self.puck.x + r >= right_line and self.puck.vx > min_goal_speed:
            if abs(self.puck.y - goal_y) < (goal_h - goal_margin):
                self.player_score += 1
    ...
    def render(self, screen):
        ...
        self.renderer.draw_player(screen, self.player, settings.HOCKEY_PLAYER_COLOR, sprite=self.renderer.player_sprite, active=True)
        self.renderer.draw_player(screen, self.ai, settings.HOCKEY_AI_COLOR, sprite=self.renderer.ai_sprite, active=False)
